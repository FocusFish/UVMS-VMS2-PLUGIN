...
import fish.focus.uvms.plugins.vms2.plugin.service.ServiceMapper;
import fish.focus.uvms.plugins.vms2.plugin.config.FileHandlerBean;
import fish.focus.uvms.plugins.vms2.plugin.config.PluginDataHolder;
import fish.focus.uvms.plugins.vms2.plugin.message.PluginMessageProducer;
...
public class StartupBean extends PluginDataHolder {
    ...
    public void startup() {
        ...
        super.setPluginApplicationProperties(fileHandler.getPropertiesFromFile(PluginDataHolder.PLUGIN_PROPERTIES));
        ...
        serviceType = ServiceMapper.getServiceType(
                ...
                getApplicationName(),
                "Plugin for receiving VMS positions.",
    ...
    public void timeout(Timer timer) {
        try {
            if (!registered && numberOfTriesExecuted < MAX_NUMBER_OF_TRIES) {
                LOG.info("{} is not registered, trying to register", getRegisterClassName());
                register();
                numberOfTriesExecuted++;
            }
            if (registered) {
                LOG.info("{} is registered. Cancelling timer.", getRegisterClassName());
                timer.cancel();
            } else if (numberOfTriesExecuted >= MAX_NUMBER_OF_TRIES) {
                LOG.info("{} failed to register, maximum number of retries reached.", getRegisterClassName());
            }
        } catch (Exception e) {
    ...
    public String getPluginApplicationProperty(String key) {
        try {
            ...
            LOG.error("Failed to get plugin property for key: {} {}", key, getRegisterClassName());
            return null;
        }
    }
    ...
    public String getSetting(String key) {
        try {
            ...
            LOG.error("Failed to get setting for key: {} {}", key, REGISTER_CLASS_NAME);
            return null;
        }
    }
